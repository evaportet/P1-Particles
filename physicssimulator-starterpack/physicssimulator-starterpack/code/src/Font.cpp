...
Font::Font(glm::vec3 pA, glm::vec3 dir, float dirMagnitude)
	: pointA(pA), spawnVel(dir), velMagnitude(dirMagnitude), Emiter(200, 1.5f)
{
	nParts = 0;
	angle = 90.f;
}
...
void Font::Update(float dt)
{
	...
	if (nParts < currentMaxPart)
	{
		...
		{
			//create particle
			glm::vec3 randomPos = pointA * (float(rand() % 100)) / 100.f;
